...
//all deliverys
$all_delivery_sql = "SELECT * FROM Delivery_arraange WHERE ar_created_by = :mails ORDER BY ar_id DESC";
$allDeliv_smtp = $conn->prepare($all_delivery_sql);
$allDeliv_smtp->bindParam(":mails" , $mail);
$allDeliv_smtp->execute();
...
<div class="table-container">
    <input type="text" id="searchInput" placeholder="Search..." style="padding:10px 20px;font-size:18px;margin-bottom: 20px;border-radius: 5px;text-decoration:none; "><br>
    <table id="dataTable">
        <thead>
        <tr>
            <th>DN REF</th>
        ...
        <tbody>
        <?php while($all_deliveries2_row = $allDeliv_smtp->fetch(PDO::FETCH_ASSOC)){ ?>
        <tr>
